...
const [, rootDir] = (0, filedirname_1.default)();
...
let VueEngine = class VueEngine extends Engine_1.Engine {
    #renderToString;
    get pronto() {
        return (0, cache_1.getCachedEngine)("pronto");
    }
    get vueify() {
        return (0, cache_1.getCachedEngine)("vueify");
    }
    get vueServerRenderer() {
        return (0, cache_1.getCachedEngine)("vue-server-renderer");
    }
    async $onInit() {
        await super.$onInit();
        await (0, cache_1.importEngine)("vue-pronto/lib/index", "pronto");
        await (0, cache_1.importEngine)("vueify");
        await (0, cache_1.importEngine)("vue-server-renderer");
    ...
    $cacheOptions(template, options, fromFile) {
        const fullPath = (0, path_1.join)(rootDir, `/${hashCode(template)}`);
        if (!fromFile) {
            return {
                ...options,
                filename: fullPath
    ...
    async getComponent(fullPath, template, options) {
        ...
        if (/<template/.test(template.trim())) {
            ...
            const compile = (0, util_1.promisify)(this.vueify.compiler.compile.bind(this.vueify.compiler));
